...
public final class WebViewJavaScriptDecoder implements YoutubeJavaScriptDecoder {
    ...
    private JSONObject evaluate(final JSONObject request) throws Exception {
        ...
        final String script = "(function(i){try{var s=performance.now(),cold="
                ...
                + "window.__ejsPrepared=r.preprocessed_player;window.__ejsPlayerId=i.playerId;"
                + "window.__ejsPlayer=null;"
                + "delete r.preprocessed_player}else{r=jsc({type:'preprocessed',"
                + "preprocessed_player:window.__ejsPrepared,requests:i.requests})}return JSON.stringify({"
